...
\begin{itemize}[leftmargin=*,itemsep=1pt,topsep=2pt]
  ...
  \item On BFCL, request-based spaces work and trace-based spaces do not: the executor fails most build conversations, so its traces show the wrong tools.
\end{itemize}
...
\begin{table}[t]
\centering\small
\caption{BFCL, 160 test records. Accuracy is the mean of three runs (individual runs in brackets). Recall: share of the reference calls' tools that keep full documentation. Tokens: executor input tokens per record, summed over turns.}
\label{tab:bfcl}
\begin{tabular}{@{}lccc@{}}
\toprule
Method & Accuracy & Recall & Tokens \\
\midrule
Full documentation & 11.3 {\scriptsize(11.3, 11.3, 11.3)} & 1.00 & 208k \\
...
\paragraph{Results.}
Table~\ref{tab:bfcl} shows the results. Both request-based settings improve on full documentation in every run, by 4 to 6 points, and cut the executor's input by more than half. \JC{} stays near full documentation. Most of the gain comes from TradingBot (30.0\% with full documentation, 41.7\% with \JA{}, 39.2\% with \JB{}) and TravelAPI (12.5\%, 15.0\%, 20.0\%).
