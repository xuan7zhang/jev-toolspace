...
\begin{itemize}[leftmargin=*,itemsep=1pt,topsep=2pt]
  ...
  \item On GTA, differences are one to three requests; request-based Jev spaces lead on two of three task types.
\end{itemize}
...
\begin{table}[t]
\centering\small
\caption{GTA accuracy (\%) with a two-tool space. One request is 3.1 points on \task{read_arith}, 6.7 on \task{count_arith}, and 5.9 on \task{web_fact}. Jev rows and the first Keep all value come from one session; the other rows come from earlier sessions on the same split, whose Keep all is given in parentheses.}
\label{tab:gta}
\begin{tabular}{@{}lccc@{}}
\toprule
Method & \task{read_arith} & \task{count_arith} & \task{web_fact} \\
\midrule
Keep all & 6.3 & 0.0 {\scriptsize(6.7)} & 0.0 {\scriptsize(0.0)} \\
...
TTO & 12.5 & 6.7 & 0.0 \\
Beam Search & 12.5 & 0.0 & 11.8 \\
\midrule
\JA{} (per request) & \textbf{15.6} & 6.7 & 0.0 \\
\JB{} (per task) & 9.4 & \textbf{20.0} & 0.0 \\
\JC{} (per task, traces) & 9.4 & 6.7 & 5.9 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Results.}
Table~\ref{tab:gta} shows the results. The 7B executor answers few GTA requests under any menu: Keep all solves 2 of 32 \task{read_arith} requests and none of the \task{web_fact} requests, so the methods differ by one to three requests. Within that range, \JA{} solves 5 of 32 \task{read_arith} requests, more than any other method, and \JB{} solves 3 of 15 \task{count_arith} requests, where every other method in Table~\ref{tab:gta} solves at most one. \JC{} has little to read: the executor calls a tool in 6 of 33 \task{read_arith} and 2 of 15 \task{count_arith} build trajectories, so most tools never appear in a trace. On \task{web_fact}, no two-tool space helps the 7B executor beyond one or two requests.
